...
create_containers() {
    ...
    for i in $(seq 1 "$num"); do
        ...
        docker run -it --rm \
            ...
            --name "$hostname" \
            centos:7 \
            /bin/bash -c "curl https://raw.githubusercontent.com/4D4937/Others/refs/heads/master/set.sh | bash && \
            curl https://raw.githubusercontent.com/4D4937/Others/refs/heads/master/zerotier-docker.sh | bash"
        # 检查容器是否创建成功
        if [ $? -eq 0 ]; then
